...
from source.model_utils import _to_one_hot
...
class DiscLogistic:
    ...
    def log_prob(self, samples):
        ...
        log_cdf_plus = plus_in - torch.functional.softplus(plus_in)
        log_one_minus_cdf_min = - torch.functional.softplus(min_in)
        cdf_delta = cdf_plus - cdf_min
        mid_in = inv_stdv * centered

        log_pdf_mid = mid_in - self.log_scales - 2. * torch.functional.softplus(mid_in)
    ...
    def sample(self):
        u = (1 - 2 * 1e-5) * torch.rand_like(self.means) + 1e-5  # B, 3, H, W
...
class DiscMixLogistic:
    def __init__(self, param, num_mix=10, num_bits=8):
        B, C, H, W = param.size()                                                     # B, M * 6 + M, H, W
        self.num_mix = num_mix
        self.logit_probs = param[:, :num_mix, :, :]                                   # B, M, H, W
        l = param[:, num_mix:, :, :].view(B, 3, 2 * num_mix, H, W)                    # B, 3, 2 * M, H, W
        self.means = l[:, :, :num_mix, :, :]                                          # B, 3, M, H, W
        self.log_scales = torch.clamp(l[:, :, num_mix:, :, :], min=-7.0)              # B, 3, M, H, W
    ...
    def log_prob(self, samples):
        ...
        log_probs = torch.sum(log_probs, 1) + torch.functional.log_softmax(self.logit_probs, dim=1)  # B, M, H, W
    ...
    def sample(self, t=1.):
        ...
        u = (1 - 2 * 1e-5) * torch.rand_like(means) + 1e-5                     # B, 3, H, W
